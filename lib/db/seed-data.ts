...
export const SEED_TENANTS: Tenant[] = [
  {
    ...
    brand_color: "#2563EB",
    logo_url: "https://rfpengine.net/logo.svg",
  ...
  },
  {
    ...
    name: "Agentic Fleet",
    subdomain: "fleet",
    tagline: "Autonomous 5-Agent SDLC Orchestration Engine",
    brand_color: "#8B5CF6",
    logo_url: "https://agentic-fleet.dev/logo.svg",
  ...
  rfqengine: [
    {
      ...
      success_metrics: ["< 3s 1-click Google Sign-in to live demo workspace", "Automatic provisioning of isolated demo tenant with pre-seeded SOC 2 documents", "Zero-trust JWT verification via Google Public JWKS on FastAPI backend", "Enterprise SAML 2.0 / Okta / Azure AD IdP federation capability"],
      acceptance_criteria: ["Users can authenticate via Google OAuth2/OIDC and receive secure session JWT.", "New demo users are automatically provisioned with pre-seeded compliance data in PostgreSQL.", "API routes enforce tenant isolation and extract user roles from Google token claims.", "Admin portal supports self-service SAML/OIDC IdP configuration via GCIP API."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Given a question, the Proposal Drafter retrieves top-k passages with semantic and keyword scores.", "When confidence is high, citations and exact source document IDs are returned.", "Responses return structured JSON with confidence score and passage attribution."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Extension listens to Background Service Worker message passing.", "Matches fields via 3-tier heuristic (exact text -> fuzzy overlap -> positional index).", "Triggers React/Angular input/change events to ensure form validation passes."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Drafter can dispatch individual or all questionnaire items to specific SME roles.", "Reviewers can leave feedback notes, request changes, or approve.", "Celebratory completion state only unlocks when all items achieve full sign-off."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Users can upload multiple files with progress feedback.", "Playground provides side-by-side prompt testing and retrieved source inspection.", "Ingested documents are queryable across all tenant sessions."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Accepts .xlsx and .csv files with auto-detection of Question and Answer columns.", "Preserves original row IDs and category groupings.", "Exports filled Excel file matching the buyer's exact column format."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Generates PDF and DOCX documents with corporate branding and table of contents.", "Appends full audit trail of reviewer approvals and timestamps.", "Includes structured appendix of all cited SOC 2 / ISO clauses."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Specialized Critic Agent cross-checks every sentence against knowledge base embeddings.", "Flags high-risk keywords (e.g. 'guarantee', 'indemnify', 'unlimited liability', '100% uptime').", "Provides inline suggested redlines with cited policy bounds."],
    ...
    },
    {
      ...
      acceptance_criteria: ["OAuth 2.0 connectors for Google Drive, Atlassian Confluence, and Notion.", "Webhook-driven incremental change indexing.", "Admin dashboard showing sync health and document freshness score."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Displays live user avatars on active question cards.", "Field locking prevents simultaneous conflicting edits.", "Inline comment threads with @mention alerts."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Extension detects and maps non-standard select2/custom dropdowns.", "Selects Yes/No/Partial based on answer context.", "Handles paginated form transitions without losing session state."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Visual charts for question frequency, confidence distributions, and turnaround times.", "Knowledge gap alert triggers when repeat questions yield low confidence scores.", "Exportable quarterly RFP performance summaries."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Given a question with review_status == 'Approved' by Security SME, Legal Counsel, or Approver.", "When the user clicks [\u2b50 Promote to Knowledge Base] in the review drawer or question card.", "Then a new KBEntry is created with category 'Golden Q&A' and metadata linking back to the origin workspace.", "And an audit badge '\u2b50 Promoted to Knowledge Base' appears on the question card."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Given a completed RFP response set with human edits.", "When responses are finalized, the system computes the edit distance (levenshtein_ratio).", "Then it logs passage attribution quality metrics in PostgreSQL.", "And if a source passage has >50% rewrite frequency across 5+ RFPs, it triggers a Stale Policy Alert in the Knowledge Hub."],
    ...
    },
    {
      ...
      acceptance_criteria: ["Given a new RFP question query.", "When search_knowledge_base executes.", "Then it retrieves both raw document chunks AND top-2 approved Golden Q&A historical pairs.", "And injects the approved pairs as dynamic few-shot exemplars inside the Gemini 2.5 Flash prompt context."],
